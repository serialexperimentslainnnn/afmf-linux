...
faq:
  ...
  - q: Does it work with Gamescope?
    a: Yes, with AFMF_GAMESCOPE=1 next to AFMF_ENABLE=1. Gamescope keeps more swapchain images in flight than a desktop compositor, so with the default 2 extra images most presents find no free image for the companion (31 percent generated in a test with gamescope 3.16 at 165 Hz); AFMF_GAMESCOPE=1 asks for 5, which gave every present one, and keeps the layer passive inside the gamescope process itself. The Steam Deck itself (RDNA2, Gamescope as the session) is untested; reports welcome.
...
compositor (Gamescope: `AFMF_GAMESCOPE=1`).
