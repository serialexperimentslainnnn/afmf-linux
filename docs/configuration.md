...
| `AFMF_MIN_FPS` | `30` | Below this real frame rate no companion is made: doubling 25 fps is not worth its latency. Generation returns once the rate is a tenth above the floor, so a game sitting on it does not flicker between the two. `0` removes the floor |
