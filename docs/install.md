...
From the [latest release](https://github.com/{{ site.repository }}/releases/latest):

| Distribution | Package |
|---|---|
| Fedora | `sudo dnf install ./afmf-linux-<version>-1.fc44.x86_64.rpm` |
| Ubuntu 24.04 / Debian trixie | `sudo apt install ./afmf-linux_<version>-1_amd64.deb` |
| Arch Linux | `sudo pacman -U afmf-linux-<version>-1-x86_64.pkg.tar.zst` (or the AUR package `afmf-linux`) |
| Anything else | `tar xf afmf-linux-<version>-x86_64.tar.xz && afmf-linux-<version>/install.sh` (installs under `~/.local`, `--uninstall` removes it) |
